...
const SESSION = [
  { t: "7:00 AM", w: "Registration opens. Sign the paper waiver and get your wristband." },
  { t: "7:30 AM", w: "Safety meeting. If you miss it, you do not drive." },
  { t: "8:00 AM", w: "The track opens for four hours, shared equally by three run groups." },
  { t: "12:00 PM", w: "The morning session ends and afternoon registration opens." },
  { t: "1:00 PM", w: "The afternoon session runs the same way until five." },
]

const BRING = [
  { k: "A helmet", v: "Required, no exceptions. You can rent one if you do not own one yet." },
  { k: "Your own car", v: "Check the fluids, pads and tires before you arrive, and take anything loose out of the cabin." },
  { k: "A signed waiver", v: "Signed on paper at registration. The marshals look for the wristband you get there." },
  { k: "Insurance, if you want it", v: "Track insurance is sold separately and is not required. Read up on it before you decide." },
]
...
export const metadata: Metadata = {
  ...
  description:
    "PaddockGavin Events track days: drive your own car on a circuit, with professional instructors and run groups by experience. The first one near Nashville is being scheduled, and autocross runs with Pro Touring Rally.",
  openGraph: {
    ...
    <>
      ...
      <main style={{ position: "relative", zIndex: 1, minWidth: 0, maxWidth: 1080, margin: "0 auto", padding: "clamp(16px,3vw,28px) clamp(12px,4vw,40px) clamp(60px,8vw,110px)", display: "flex", flexDirection: "column", gap: "clamp(44px,6vw,84px)" }}>
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: "clamp(16px,2.4vw,26px)" }}>
          <PGEEyebrow>PaddockGavin Events &middot; Nashville</PGEEyebrow>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, fontFamily: ARCHIVO, fontSize: "var(--t-lead)", lineHeight: 1.62, color: "#C4CBD6", maxWidth: "62ch" }}>
            <p style={{ margin: 0 }}>
              A track day puts you in your own car on a circuit, with someone beside you who knows
              the line. The first one near Nashville is being scheduled now, and it will be the
              first we have run here.
            </p>
            <p style={{ margin: 0 }}>
              You do not need a race car. You need the car you already own, a helmet and a morning.
              This page covers the day hour by hour and what to bring.
            </p>
          </div>
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: "clamp(16px,2.2vw,26px)" }}>
          ...
          <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: "var(--t-lead)", lineHeight: 1.6, color: "#C4CBD6", maxWidth: "62ch" }}>
            There are two sessions, with twenty five cars in each. Drivers split into three run
            groups by experience, and every group gets the same track time. Pick the group that
            honestly fits you. Nobody minds you starting in the first one.
          </p>
          <ol className="pg-e1" style={{ margin: 0, padding: "clamp(8px,1.2vw,14px) clamp(16px,2.2vw,24px)", listStyle: "none", clipPath: NOTCH }}>
            {SESSION.map((s, i) => (
              <li key={s.t} style={{ display: "flex", flexWrap: "wrap", gap: "2px 18px", padding: "14px 0", borderTop: i === 0 ? "none" : "1px solid rgba(255,255,255,.09)" }}>
                <span style={{ fontFamily: MONO, fontSize: 14, fontVariantNumeric: "tabular-nums", letterSpacing: ".04em", color: PGE.teal, flex: "0 0 84px" }}>{s.t}</span>
          ...
          <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: 15.5, lineHeight: 1.6, color: "#9AA4B2", maxWidth: "62ch" }}>
            Spectators are welcome in standing room, but only drivers go on track. It runs in the
            rain.
          </p>
        </section>
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: "clamp(16px,2.2vw,26px)" }}>
          ...
          <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: "var(--t-lead)", lineHeight: 1.6, color: "#C4CBD6", maxWidth: "62ch" }}>
            Every tier gets you the same four hours on track. What changes is what happens between
            sessions.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(250px,100%),1fr))", gap: "clamp(12px,1.6vw,18px)" }}>
            {TIERS.map((t, i) => (
              <div key={t.name} className={i === 2 ? "pg-e2" : "pg-e1"} style={{ display: "flex", flexDirection: "column", gap: 12, padding: "clamp(18px,2.4vw,26px)", clipPath: NOTCH }}>
                ...
                <p style={{ margin: 0, fontFamily: MONO, fontSize: "var(--t-small)", letterSpacing: ".08em", textTransform: "uppercase", color: PGE.teal }}>{t.line}</p>
          ...
          <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: 15.5, lineHeight: 1.6, color: "#9AA4B2", maxWidth: "62ch" }}>
            {FIRST
              ...
              : "No prices yet. The cost depends on the circuit, and that is still being settled. Prices post here once they are set."}
          </p>
        </section>

        {/* Autocross */}
        <section style={{ display: "flex", flexDirection: "column", gap: "clamp(16px,2.2vw,26px)" }}>
          <Eyebrow>With Pro Touring Rally</Eyebrow>
          <h2 style={{ margin: 0, fontFamily: ARCHIVO, fontWeight: 800, fontSize: "var(--t-h2)", lineHeight: 1.05, letterSpacing: "-.022em", color: "#FFFFFF", textWrap: "balance" }}>
            Autocross, an easier first step
          </h2>
          <div className="pg-e1" style={{ display: "flex", flexDirection: "column", gap: 14, padding: "clamp(18px,2.6vw,28px)", clipPath: NOTCH }}>
            <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: "var(--t-lead)", lineHeight: 1.62, color: "#C4CBD6", maxWidth: "62ch" }}>
              Autocross is a cone course on a big flat surface, one car at a time, mostly in second
              gear. It is slower than a track day but teaches you more per minute. You run the same
              corners until you get them right.
            </p>
            <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: "var(--t-lead)", lineHeight: 1.62, color: "#C4CBD6", maxWidth: "62ch" }}>
              It is also the cheapest way to find out whether you like driving fast, in the car you
              already own. We run these with Pro Touring Rally.
            </p>
            <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: 15.5, lineHeight: 1.6, color: "#9AA4B2" }}>
              Ask me and I will tell you when the next one is.
            </p>
          </div>
        </section>
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: "clamp(14px,2vw,22px)" }}>
          ...
          <h2 style={{ margin: 0, fontFamily: ARCHIVO, fontWeight: 800, fontSize: "var(--t-h2)", lineHeight: 1.05, letterSpacing: "-.022em", color: "#FFFFFF", textWrap: "balance" }}>
            {FIRST ? "It is booked" : "Get in the first group"}
          </h2>
          <p style={{ margin: 0, fontFamily: ARCHIVO, fontSize: "var(--t-lead)", lineHeight: 1.6, color: "#C4CBD6", maxWidth: "62ch" }}>
            {FIRST
              ? `${FIRST.date}, at ${FIRST.venue}.`
              : "Each session takes twenty five cars, and the people who ask early get the morning run group. Tell me what you drive and how much track time you have had. I will tell you which group fits, and when the date lands."}
          </p>
