...
export function gateEnforced() {
  /* A Cloudflare secret is an 0x string of about thirty five characters. The
     one this estate had set was eleven characters with no prefix, and
     siteverify answers invalid-input-secret for it, so the gate never
     filtered a bot; it only closed. A key that cannot pass is treated as no
     key, here and on the endpoint that reads it. */
