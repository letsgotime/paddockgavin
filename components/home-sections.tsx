...
/** The products, as a list, not a paragraph with a plug in it. */
export function ProductRows() {
  return (
    <div className="pg-e0" style={{ display: "flex", flexDirection: "column", padding: "4px 18px", clipPath: "polygon(0 0,100% 0,100% calc(100% - 12px),calc(100% - 12px) 100%,0 100%)" }}>
      {PRODUCTS.map((r, i) => {
        const style: React.CSSProperties = { display: "flex", alignItems: "baseline", gap: 12, padding: "12px 0", borderBottom: i < PRODUCTS.length - 1 ? "1px solid rgba(255,255,255,.08)" : 0, textDecoration: "none" }
        const inner = (
          <>
        ...
        return r.external
          ? <a key={r.key} href={r.href} target="_blank" rel="noopener noreferrer" style={style}>{inner}</a>
          : <Link key={r.key} href={r.href} style={style}>{inner}</Link>
      })}
    </div>
  )
}
